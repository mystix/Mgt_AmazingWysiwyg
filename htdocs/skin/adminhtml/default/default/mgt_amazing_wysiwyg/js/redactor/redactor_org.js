...
Redactor.prototype = {
    ...
    {    
        ...
        else if (this.opts.toolbar !== false)
        {
            ...
            $j.extend(this.opts.toolbar, this.opts.buttonsCustom);            
            $j.each(this.opts.buttonsAdd, $j.proxy(function(i,s)
            {
    ...
    build: function(mobile)
    {
        ...
        {        
            // container
            this.$box = $j('<div class="redactor_box"></div>');
    ...
    buildToolbar: function()
    {
        ...
        this.$toolbar = $j('<ul>').addClass('redactor_toolbar');
        ...
        $j.each(this.opts.buttons, $j.proxy(function(i,key)
        {
            ...
            if (key !== '|' && typeof this.opts.toolbar[key] !== 'undefined') 
            {
                ...
                if (key === 'backcolor' || key === 'fontcolor' || typeof(s.dropdown) !== 'undefined')
                {
                    ...
                    this.dropdowns.push(dropdown.appendTo($j(document.body)));
                ...
                this.$toolbar.append($j(li).append(a));
            }
    ...
    {
        var button = $j('<a href="javascript:void(null);" title="' + s.title + '" class="redactor_btn_' + key + '"></a>');
        
        if (typeof s.func === 'undefined')
        {
            button.click($j.proxy(function() { this.execCommand(s.exec, key); }, this));
        }
        else if (s.func !== 'show')
        {
            button.click($j.proxy(function(e) {
        ...
        $j.each(obj, $j.proxy(
            function (x, d)
            {
                ...
                else
                {
                    ...
                    {
                        $j(drop_a).click($j.proxy(function() { this.execCommand(d.exec, x); }, this));
                    }
                    else
                    {
                        $j(drop_a).click($j.proxy(function(e) { this[d.func](e); }, this));
                    }
                }

                $j(dropdown).append(drop_a);
